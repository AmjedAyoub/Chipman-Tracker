...
class Logging extends Component {
    ...
    handleFormSubmit = event => {
        ...
        if( this.state.password2 === this.state.password1){
            axios.post("http://chipmantrack.herokuapp.com/signup", { user: this.state.user, email: this.state.email, password: this.state.password1 })
                .then(res=> {
    ...
    handleSginInSubmit = event => {
        // Preventing the default behavior of the form submit (which is to refresh the page)
        event.preventDefault();
        axios.post("http://chipmantrack.herokuapp.com/login", { email: this.state.logemail, password: this.state.logpassword })
            .then(res => {
                if(res.data.msg){
                        this.setState({ 
                            isAuthenticated: false,
